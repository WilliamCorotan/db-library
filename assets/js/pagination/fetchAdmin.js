function fetchAdmin(pageNumber = 1){
    const actions =  function(userType){ return (`
<div class="text-center">
    <span class="edit-${userType} h-100"> <i class="fa-solid fa-pencil fa-md"></i> </span>      
</div>
`)}
    let search = ($('#admin-search').val())? $('#admin-search').val() : null;
    $(document).ready(function () {
        $.ajax({
            type: "get",
            url: `http://localhost/admin/fetch/admins/${pageNumber}/${search}`,
            dataType: "json",
            beforeSend: function(){
            ...
            success: function (response) {
                ...
                if(response.total_pages != 1){
                    ...
                    $('.pagination').on('click', '.page-link', function(){
                        fetchAdmin($(this).attr('data-page-number'))
                    })
                }
            }
        });
